//https://www.e-olymp.com/ru/problems/2804
#include <bits/stdc++.h>
using namespace std;
...
void check(int n, int &mxprime)
{
    if(prime(n) && n > mxprime)
        mxprime = n;
    ...
    {
        int cur = stoi(s.substr(0, i) + s.substr(i + 1));
        if(cur > 0)
            check(cur, mxprime);
    }
}
...
void solve()
{
    ...
    //ld l = sqrt(sum);
    //ld l2 = trunc(l);
    cout << (sqrt(sum) == trunc(sqrt(sum)) ? "Yes" : "No") << endl;
}
