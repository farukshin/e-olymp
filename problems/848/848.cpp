// https://www.e-olymp.com/ru/problems/848
#include <bits/stdc++.h>
using namespace std;
...
void backtraking(int pos, ll cur, ll &n, vector<ll> &v)
{
    if(pos == v.size())
    {
        curAns.insert(cur);
    ...
    backtraking(pos + 1, cur, n, v);
    backtraking(pos + 1, cur * v[pos], n, v);
}
...
void solve()
{
    ...
    for(ll i = a; i <= b; i++)
    {
        ...
        map<ll, ll> result;
        ll unused;
        factorize(i, result, unused);

        if(result.size() == 0 || result.size() == 1 && (*result.begin()).first == i )
            continue;

        vector<ll> v;
        for(auto d : result)
            if(d.first != i)
                for(int kk = 1; kk <= d.second; kk++)
                    v.push_back(d.first);
