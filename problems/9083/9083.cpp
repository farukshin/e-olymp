//https://www.e-olymp.com/ru/problems/9083
#include <bits/stdc++.h>
using namespace std;
    ...
    while(!q.empty())
    {
        ...
        if(d(dot, finish) > 10)
        {
            if(dot.second > finish.second)
                used.insert({dot.first + 2, dot.second - 1}), q.push({len + 1, {dot.first + 2, dot.second - 1}});
            else
                used.insert({dot.first + 2, dot.second + 1}), q.push({len + 1, {dot.first + 2, dot.second + 1}});
            continue;
        }
        ...
        for(auto r : run)
        {
            ...
            if(d(dot, finish) + 1 >= d(next, finish) && !used.count(next))
